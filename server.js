...
async function bsdFetch(path) {
  const res = await fetch(BSD_BASE + path, { headers: { "Authorization": "Token " + BSD_API_KEY } });
  if (!res.ok) throw new Error("BSD error " + res.status + ": " + path);
  return res.json();
}
...
function normalizeBSDEvent(ev, pred) {
  ...
  return {
    ...
    p: {
      ...
      dc1x: Math.min(96, homeProb + drawProb), dcx2: Math.min(96, awayProb + drawProb),
      over05: Math.min(96, over15 + 8), over15: over15, over25: over25,
      over35: Math.max(18, over25 - 22), u25: Math.max(20, 100 - over25), u35: 72,
...
function buildAlerts(matches) {
  ...
  matches.forEach(function(m) {
    ...
    if (m.p.over15 >= 82 && m.p.gg >= 60) alerts.push({ m:m, tag:"✅ OVER 1.5 SOLIDO", score:score, text:"Over 1.5 all'" + m.p.over15 + "% con GG al " + m.p.gg + "%" });
    else if (m.p.over25 >= 65) alerts.push({ m:m, tag:"🔥 OVER 2.5 FORTE", score:score, text:"Over 2.5 al " + m.p.over25 + "%" });
    else if (m.p.gg >= 68 && m.p.over15 >= 78) alerts.push({ m:m, tag:"⚽ GG CONFERMATO", score:score, text:"GG al " + m.p.gg + "%" });
    else if (m.p.dc1x >= 80 && m.p.h >= 50) alerts.push({ m:m, tag:"🛡 DC 1X SICURA", score:score, text:"DC 1X all'" + m.p.dc1x + "%" });
  });
...
function scheduleDailyJob() {
  ...
  setTimeout(function() { sendDailyAlerts(); setInterval(sendDailyAlerts, 24*60*60*1000); }, msUntil);
}

async function autoFetchOnStartup() {
  const date = localDateRome(0);
  const key = getCacheKey(date);
  // Svuota sempre la cache all'avvio per garantire dati freschi
  cache.clear();
  console.log("[Startup] Cache svuotata. Caricamento partite per " + date + "...");
  try {
    let matches, source;
    if (USE_MOCK) { matches = generateMockMatches(date); source = "mock"; }
    else { matches = await fetchBSDMatches(date); source = "bsd"; }
    cache.set(key, { createdAt: Date.now(), matches: matches, source: source });
...
app.get("/api/health", function(req, res) {
  res.json({ ok:true, provider:USE_MOCK?"MOCK":"BSD", useMock:USE_MOCK, bsdConfigured:Boolean(BSD_API_KEY), telegramConfigured:Boolean(TELEGRAM_TOKEN), cacheTtlMinutes:CACHE_TTL_MINUTES, cacheEntries:cache.size });
});

app.get("/api/debug/bsd", async function(req, res) {
  ...
  try {
    ...
    res.json({ ok:true, totalCount:data.count, keys:Object.keys(ev), sample:ev });
  } catch(e) { res.status(500).json({ ok:false, error:e.message }); }
});

app.get("/api/debug/pred", async function(req, res) {
  if (USE_MOCK) return res.json({ ok:false, error:"Modalità mock attiva" });
  try {
    const date = localDateRome(0);
    const data = await bsdFetch("/events/?date_from=" + date + "&date_to=" + date + "&limit=1");
    const ev = data.results && data.results[0];
    if (!ev) return res.json({ ok:false, error:"Nessuna partita" });
    const pred = await bsdFetch("/predictions/?event=" + ev.id);
    const p = (pred.results && pred.results[0]) || {};
    res.json({ ok:true, eventId:ev.id, predKeys:Object.keys(p), pred:p });
  } catch(e) { res.status(500).json({ ok:false, error:e.message }); }
});
...
app.get("/api/telegram/test", async function(req, res) {
  ...
  res.json({ ok:true, message:"Messaggio di test inviato" });
});

app.get("/api/telegram/send-now", async function(req, res) {
  if (!TELEGRAM_TOKEN) return res.json({ ok:false, error:"TELEGRAM_TOKEN non configurato" });
  await sendDailyAlerts();
  res.json({ ok:true, message:"Alert inviati" });
});
...
app.get("/api/matches/today", async function(req, res) {
  try {
    ...
    if (isCacheValid(cached)) {
      return res.json({ date:date, count:cached.matches.length, matches:cached.matches, source:cached.source||"cache", cache:{ hit:true } });
    }
    let matches, source;
    if (USE_MOCK) { matches = generateMockMatches(date); source = "mock"; }
    else { matches = await fetchBSDMatches(date); source = "bsd"; }
    cache.set(key, { createdAt:Date.now(), matches:matches, source:source });
    res.json({ date:date, count:matches.length, matches:matches, source:source, cache:{ hit:false } });
  } catch(error) {
    const date = req.query.date || localDateRome(0);
    const cached = cache.get(getCacheKey(date));
    if (cached) return res.json({ date:date, count:cached.matches.length, matches:cached.matches, source:"stale-cache", warning:error.message });
...
app.listen(PORT, HOST, function() {
  ...
  console.log("Modalità: " + (USE_MOCK ? "MOCK" : "BSD Free API"));
